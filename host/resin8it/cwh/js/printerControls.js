(function() {
	...
	cwhApp.controller("PrinterControlsController", ['$scope', '$http', '$location', '$routeParams', 'cwhWebSocket', 'photonicUtils', function ($scope, $http, $location, $routeParams, cwhWebSocket, photonicUtils) {
        ...
        this.moveZ = function moveZ(step){	
        	$http.get("services/printers/executeGCode/" + printerName + "/G1 Z" + step + " F300").then(gCodeSuccess, errorFunction)
        }
        this.home = function home(dimension) {
			$http.get("services/printers/executeGCode/" + printerName + "/G28 Z0").then(gCodeSuccess, errorFunction)
		}
        ...
        this.showLogo = function showLogo() {
			$http.get("services/printers/showLogo/" + printerName).then(gCodeSuccess, errorFunction)
		}
